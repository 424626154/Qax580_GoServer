...
func (c *AppController) AddKnowBrew() {
	var res = models.ResponseJson{}
	if c.Ctx.Input.IsGet() {
		beego.Debug("APP AddKnowBrew Get")
		res.Errcode = APP_ERRCODE1003
		res.Errmsg = APP_ERRMSG1003
	}
	if c.Ctx.Input.IsPost() {
		beego.Debug("APP AddKnowBrew Post")
		req_json := string(c.Ctx.Input.RequestBody)
		token := gojson.Json(req_json).Get("token").Tostring()
		kbname := gojson.Json(req_json).Get("kbname").Tostring()
		brief := gojson.Json(req_json).Get("brief").Tostring()
		link := gojson.Json(req_json).Get("link").Tostring()
		kbtype := gojson.Json(req_json).Get("type").Tostring()
		beego.Debug("req_json:", req_json)
		beego.Debug("kbname:", kbname)
		if len(token) > 0 && len(kbname) > 0 && len(link) > 0 {
			user, err := models.GetOneUserToken(token)
			if err != nil {
				beego.Error(err)
			}
			if user != nil {
				...
				beego.Debug("ikbtype:", ikbtype)
				obj, err := models.AddKnowBrew(kbname, brief, link, user.Uid, user.Username, int8(ikbtype))
				if err != nil {
					beego.Error(err)
				}
				res.Errcode = 0
...
func (c *AppController) KnowBrew() {
	var res = models.ResponseJson{}
	if c.Ctx.Input.IsGet() {
		beego.Debug("APP KnowBrew Get")
		res.Errcode = APP_ERRCODE1003
		res.Errmsg = APP_ERRMSG1003
	}
	if c.Ctx.Input.IsPost() {
		beego.Debug("APP KnowBrew Post")
		// req_json := string(c.Ctx.Input.RequestBody)
		// token := gojson.Json(req_json).Get("token").Tostring()
	...
	beego.Debug("KnowBrew res_json:", string(res_json))
	c.Ctx.WriteString(string(res_json))
}

func (c *AppController) Business() {
	var res = models.ResponseJson{}
	if c.Ctx.Input.IsGet() {
		beego.Debug("APP Business Get")
		res.Errcode = APP_ERRCODE1003
		res.Errmsg = APP_ERRMSG1003
	}
	if c.Ctx.Input.IsPost() {
		beego.Debug("APP Business Post")
		// req_json := string(c.Ctx.Input.RequestBody)
		// token := gojson.Json(req_json).Get("token").Tostring()
		objs, err := models.GetBusinessState(1)
		if err != nil {
			beego.Error(err)
		}
		res.Errcode = 0
		res.Errmsg = "获取优秀商家列表成功"
		data_json, err := json.Marshal(objs)
		if err != nil {
			beego.Error(err)
		}
	...
	beego.Debug("Business res_json:", string(res_json))
	c.Ctx.WriteString(string(res_json))
}
